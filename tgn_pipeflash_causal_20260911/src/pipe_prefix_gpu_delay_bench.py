...
def main():
    ...
    p.add_argument('--audit-state',action='store_true')
    ...
    args=p.parse_args();args.delay_kind='cuda_stream';args.prefix_snapshot=True;args.inline_state_send=False;world=int(os.environ.get('WORLD_SIZE','1'));rank=int(os.environ.get('RANK','0'))
    ...
    if args.delay_ms:
        assert source.count('time.sleep('+repr(args.delay_ms/1000)+')')==2
        source=source.replace('time.sleep('+repr(args.delay_ms/1000)+')','record_gpu_delay('+repr(args.delay_ms)+')')
    # Every rank finishes the shared state reset before rank 0 starts the pass.
    import re
    source,n_reset=re.subn(r'(?m)^([ ]+)model.reset\(\)$',lambda m:m.group(0)+'\n'+m.group(1)+'dist.barrier()',source)
    if n_reset!=2:raise RuntimeError(('reset sites',n_reset))
    # Defer evaluation release on the main thread so it cannot miss the snapshot.
    pattern=r'(?m)^([ ]+)sends_thread = threading.Thread\(target=send, args=\(None, dst, groups\[idx\]\)\)\n\1sends_thread.start\(\)'
    source,n_eval=re.subn(pattern,lambda m:m.group(1)+'send(None,dst,groups[idx])\n'+m.group(1)+'sends_thread = None',source)
    if n_eval!=1:raise RuntimeError(('evaluation release site',n_eval))
    if args.schedule=='state_first':
        source,n_sample=re.subn(r'(?m)^([ ]+)if has_next: sampling\(next_target_nodes, next_ts, next_eid\)$',lambda m:m.group(1)+'# Sampling follows the support snapshot release.',source)
        if n_sample!=2:raise RuntimeError(('sampling sites',n_sample))
        pattern=r'(?m)^([ ]+)model.prepare_input\(b, updated_memory, overlap_nid\)$'
        source,n_prep=re.subn(pattern,lambda m:m.group(0)+'\n'+m.group(1)+'if has_next: sampling(next_target_nodes, next_ts, next_eid)',source)
        if n_prep!=2:raise RuntimeError(('support prepare sites',n_prep))
    if args.freeze_weights:
        source=source.replace('    args.lr = args.lr * math.sqrt(args.world_size)','    args.lr = 0.0')
    ...
    from prefix_snapshot import PrefixSnapshot
    prefix_protocol=PrefixSnapshot(ns,TGNN,Memory,out,inline=args.inline_state_send,audit=args.audit_state)
    def wrap_phase(name):
        original=ns[name]
    ...
    from cuda_delay import CudaDelay
    cuda_delay=CudaDelay(out,phase)
    ns['record_gpu_delay']=cuda_delay
