...
def main():
    ...
    args=p.parse_args();args.state_send_launch_fence=True;args.parameter_transport='arena';world=int(os.environ.get('WORLD_SIZE','1'));rank=int(os.environ.get('RANK','0'))
    ...
    if args.schedule=='state_first':
        import re
        marker=r'(?m)^([ ]+)(updated_memory, overlap_nid, sends_thread1 = model.update_memory_and_send.*)$'
        source,n=re.subn(marker,lambda m:m.group(0)+'\n'+m.group(1)+'if sends_thread1 is not None: sends_thread1.join()',source)
        if n!=2:raise RuntimeError('Expected two state launch fences')
