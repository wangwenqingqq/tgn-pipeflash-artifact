...
__global__ void fused_gather_encode_bwd_v2_kernel(
    ...
    for (int edge = global_warp; edge < total_edges; edge += stride) {
        ...
        for (int t = lane; t < d_t; t += 32) {
            ...
            float neg_sin = -sinf(fmaf(delta, s_te_w[t], s_te_b[t]));
...
__global__ void fused_gather_encode_v2_kernel(
    ...
    for (int edge = global_warp; edge < total_edges; edge += stride) {
        ...
        for (int f = lane; f < d_t; f += 32)
            z_row[d + d_e + f] = cosf(fmaf(delta, s_te_w[f], s_te_b[f]));
    }
...
fused_gather_encode_v2(
    ...
    fused_gather_encode_v2_kernel<<<n_blocks, threads, smem_bytes, at::cuda::getCurrentCUDAStream()>>>(
...
fused_gather_encode_backward_v2(
    ...
    fused_gather_encode_bwd_v2_kernel<<<n_blocks, threads, smem_bytes, at::cuda::getCurrentCUDAStream()>>>(
